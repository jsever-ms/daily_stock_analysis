...
- [测试] `test_bot_status_command.py` 新增 `test_status_command_reports_anspire_search_status` 验证 Anspire 搜索状态显示
- [新功能] 新增 `.github/workflows/anspire-smoke.yml` + `.github/scripts/test_anspire_smoke.py`：手动触发的 Anspire Search API 连通性 smoke test，不运行股票分析、不调用 LLM
...
- [修复] 修复 Fast Pipeline 最终 LLM summary 调用异常（AI总结 0.0s）：原代码在 `_fast_pipeline_analyze` 中直接使用 `litellm.completion()` 绕过项目统一的 `LLMToolAdapter` 通道路由/API key 注入，导致 `LITELLM_MODEL=gemini/gemini-3.1-pro-preview` 和 `LLM_CHANNELS=gemini,siliconflow` 配置未生效，最终调用无正确 API key 时立即异常。改为复用 `LLMToolAdapter.call_text()` 走项目现有通道路由；新增 `LLM_CALL_FAILED` / `LLM_RESPONSE_PARSE_FAILED` 分类日志；Telegram 失败时仍保留原始数据 fallback 不暴露 traceback。
- [测试] 新增 `tests/test_fast_pipeline_summary.py`：mock 正常 LLM 返回时验证生成五段式简报而非 fallback；mock provider=error / 空内容 / 短内容 / 异常时验证 fallback 至原始数据。
- [改进] Fast Pipeline 新增 `ASK_FAST_MODEL` 独立配置：/ask <代码> 默认快速问股最终摘要优先使用此模型，空则继承 `LITELLM_MODEL`；不影响 `/ask detail`、`/analyze`、`/research`、`AGENT_LITELLM_MODEL`。`/status detail` 增加「快速问股模型」显示。
...
- [改进] `/analyze` 详细报告固定为九段式结构（① 决策仪表盘 ② 当前操作计划 ③ 关键价位与风险收益 ④ 技术与量价 ⑤ 基本面与估值 ⑥ 新闻/公告/催化与风险 ⑦ 条件检查 ⑧ 下一观察点 ⑨ 数据完整性），同一观点只在一处展开、其余模块引用，消除「资金与量价」等段落重复表述；模板与 notification 旧版渲染路径同步对齐。
- [改进] 偏空结论决策口径全局一致：最终动作为减仓/卖出/看空时，空仓者不再出现买入点/试仓点/轻仓试探等表述，操作点位表改为「观察区 + 转强确认条件」，并对 `position_advice.no_position` 与 `entry_plan` 做入场话术清洗（`sanitize_bearish_entry_text`）；仅趋势恢复中性且必要条件满足后才允许重新出现试仓建议。
- [改进] 信号归因移除无量化算法支撑的百分比权重（55%/20%/15%/10%），改为「主导因素 / 主要看多因素 / 主要看空因素」定性归因，模板、notification 旧路径与 schema 同步更新。
- [改进] 关键价位区分「技术参考位（支撑/压力）」与「操作执行位（止损/目标）」并附说明；盈亏比改为程序化计算（现价/止损/目标推算，比例<1 时提示「当前开仓性价比不足」），不由 LLM 心算。
- [改进] 新闻事实保留日期与来源；下一观察点补充行动窗口（盘中决策护栏）与置信度理由展示，数据限制归入数据完整性段落；补齐韩语标签保证三语言报告结构一致。
- [测试] 新增/更新空头场景条件渲染、定性归因、程序化盈亏比、九段式顺序与 phase_decision 展示等回归测试。
